...
Nodes first connect through the compiled DNS seed, advertise the local listening
address used by that connection, exchange addresses with `getaddr`/`addrv2`, and
dynamically dial newly learned peers. No restart or private-LAN `-addnode` is
needed when the discovered address is reachable.

Use `-debug=addrman -debug=net` to watch discovery. Seed-only nodes may use
`--max-outbound 0`; normal clients must leave outbound connections enabled.
