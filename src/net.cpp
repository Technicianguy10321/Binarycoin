...
PeerVersion local_version(const TestnetChain& chain, const std::uint64_t fixed_nonce = 0) {
    ...
    return PeerVersion{
        ...
        .user_agent = "/BinaryCoinTestnetAlpha:0.1.5/",
    };
}
...
std::string endpoint_key(const PeerEndpoint& endpoint) {
    return endpoint.host + "\n" + std::to_string(endpoint.port);
}

bool advertisable_address(const std::string& host, const bool allow_loopback) {
    in_addr ipv4{};
    if (::inet_pton(AF_INET, host.c_str(), &ipv4) == 1) {
        const std::uint32_t value = ntohl(ipv4.s_addr);
        const std::uint8_t first = static_cast<std::uint8_t>((value >> 24U) & 0xffU);
        const std::uint8_t second = static_cast<std::uint8_t>((value >> 16U) & 0xffU);
        if (first == 0 || first >= 224) return false;
        if (first == 127) return allow_loopback;
    ...
    if (::inet_pton(AF_INET6, host.c_str(), &ipv6) == 1) {
        if (IN6_IS_ADDR_UNSPECIFIED(&ipv6) || IN6_IS_ADDR_MULTICAST(&ipv6) ||
            IN6_IS_ADDR_LINKLOCAL(&ipv6)) return false;
        if (IN6_IS_ADDR_LOOPBACK(&ipv6)) return allow_loopback;
        return true;
    }
    return false;
}

PeerEndpoint listening_endpoint_for_socket(const Socket socket, const std::uint16_t listen_port) {
    sockaddr_storage local{};
    SocketLength length = sizeof(local);
    if (::getsockname(socket, reinterpret_cast<sockaddr*>(&local), &length) != 0) {
        return {};
    }
    PeerEndpoint endpoint = endpoint_from_sockaddr(local);
...
PersistentTestnetNode::PersistentTestnetNode(
    ...
    if (policy_.ban_threshold == 0 || policy_.max_messages_per_window == 0 ||
        ...
        policy_.address_refresh_interval <= std::chrono::milliseconds::zero()) {
...
std::vector<PeerEndpoint> PersistentTestnetNode::initial_outbound_peers() {
    ...
    {
        std::lock_guard lock(local_endpoints_mutex_);
        for (const std::string& key : local_endpoint_keys_) {
            const std::size_t separator = key.rfind('\n');
            if (separator == std::string::npos) continue;
            excluded.push_back(PeerEndpoint{
                .host = key.substr(0, separator),
                .port = static_cast<std::uint16_t>(std::stoul(key.substr(separator + 1))),
            });
        }
    }
    ...
    PeerStore peer_store(data_directory_);
    peer_store.load();
    return peer_store.select_outbound(64, excluded);
}
...
void PersistentTestnetNode::remember_local_endpoint(const PeerEndpoint& endpoint) {
    if (endpoint.host.empty() || endpoint.port == 0) return;
    std::lock_guard lock(local_endpoints_mutex_);
    local_endpoint_keys_.insert(endpoint_key(endpoint));
}

bool PersistentTestnetNode::is_local_endpoint(const PeerEndpoint& endpoint) const {
    if (endpoint.host.empty() || endpoint.port == 0) return true;
    std::lock_guard lock(local_endpoints_mutex_);
    return local_endpoint_keys_.contains(endpoint_key(endpoint));
}
...
void PersistentTestnetNode::start() {
    ...
    if (policy_.max_outbound == 0) {
        log_info("net", "Selected 0 outbound peer(s)");
    } else {
        outbound_manager_thread_ = std::thread([this] { outbound_manager_loop(); });
    }
}
...
void PersistentTestnetNode::wait() {
    ...
    if (outbound_manager_thread_.joinable()) {
        log_debug("net", "joining outbound manager port=" + std::to_string(port_));
        outbound_manager_thread_.join();
    }
    if (listen_socket_ != INVALID_SOCKET_VALUE) {
        close_socket(listen_socket_);
        listen_socket_ = INVALID_SOCKET_VALUE;
    ...
    std::vector<OutboundThread> outbound;
    {
        std::lock_guard lock(outbound_threads_mutex_);
        outbound.swap(outbound_threads_);
        outbound_targets_.clear();
    }
    log_debug("net", "joining outbound threads port=" + std::to_string(port_) +
        " count=" + std::to_string(outbound.size()));
    for (OutboundThread& worker : outbound) {
...
void PersistentTestnetNode::reap_outbound_threads() {
    std::vector<OutboundThread> completed;
    {
        std::lock_guard lock(outbound_threads_mutex_);
        auto iterator = outbound_threads_.begin();
        while (iterator != outbound_threads_.end()) {
            if (!iterator->done->load()) {
                ++iterator;
                continue;
            }
            outbound_targets_.erase(endpoint_key(iterator->endpoint));
            completed.push_back(std::move(*iterator));
            iterator = outbound_threads_.erase(iterator);
        }
    }
    for (OutboundThread& worker : completed) {
        if (worker.thread.joinable()) worker.thread.join();
    }
}

void PersistentTestnetNode::start_outbound_target(PeerEndpoint endpoint) {
    if (stop_requested_ || policy_.max_outbound == 0 || endpoint.host.empty() || endpoint.port == 0) return;
    if (is_local_endpoint(endpoint)) return;
    ...
    {
        std::lock_guard lock(outbound_threads_mutex_);
        if (outbound_threads_.size() >= policy_.max_outbound || outbound_targets_.contains(key)) return;
        const auto retry = outbound_retry_after_.find(key);
        if (retry != outbound_retry_after_.end() && retry->second > std::chrono::steady_clock::now()) return;
        outbound_targets_.insert(key);
        log_info("addrman", "Selected discovered outbound peer " + endpoint.host + ':' +
            std::to_string(endpoint.port));
        worker.thread = std::thread([this, endpoint = std::move(endpoint), done] {
            outbound_loop(endpoint);
            done->store(true);
        });
        outbound_threads_.push_back(std::move(worker));
    }
}

void PersistentTestnetNode::outbound_manager_loop() {
    while (!stop_requested_) {
        reap_outbound_threads();
        const auto selected = initial_outbound_peers();
        for (const PeerEndpoint& endpoint : selected) {
            if (stop_requested_) break;
            start_outbound_target(endpoint);
            std::lock_guard lock(outbound_threads_mutex_);
            if (outbound_threads_.size() >= policy_.max_outbound) break;
        }
...
void PersistentTestnetNode::outbound_loop(PeerEndpoint endpoint) {
    ...
    if (peer_is_banned(endpoint)) {
        ++banned_connections_;
        std::lock_guard lock(outbound_threads_mutex_);
        outbound_retry_after_[key] = std::chrono::steady_clock::now() + std::chrono::seconds(10);
        return;
    }
    ...
    try {
        ...
        std::lock_guard lock(outbound_threads_mutex_);
        outbound_retry_after_[key] = std::chrono::steady_clock::now() + std::chrono::seconds(2);
        ...
        std::uint32_t failures = 1;
        {
            ...
            for (const PeerRecord& record : peer_store.records()) {
        ...
        const auto backoff = std::chrono::seconds(std::min<std::uint32_t>(60, 1U << std::min<std::uint32_t>(failures, 5)));
        std::lock_guard lock(outbound_threads_mutex_);
        outbound_retry_after_[key] = std::chrono::steady_clock::now() + backoff;
    }
}
...
void PersistentTestnetNode::session_loop(
    ...
    auto advertise_self = [&] {
        const PeerEndpoint local = listening_endpoint_for_socket(socket.get(), port_);
        if (!advertisable_address(local.host, policy_.allow_loopback_discovery_for_tests)) return;
        remember_local_endpoint(local);
        const auto now = static_cast<std::int64_t>(
            std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()));
        ...
        send_message(socket.get(), "addrv2", serialize_addrv2(records));
        log_debug("addrman", "Advertised local endpoint " + local.host + ':' +
            std::to_string(local.port) + " to peer=" + std::to_string(peer_id));
    };
    ...
    auto last_address_refresh = last_ping;
    ...
    while (!stop_requested_) {
        ...
        try {
            if (socket_readable(socket.get(), 100)) {
                ...
                if (message.command == "addrv2") {
                    ...
                    for (PeerRecord record : records) {
                        if (!advertisable_address(
                                record.endpoint.host,
                                policy_.allow_loopback_discovery_for_tests)) continue;
                        if (is_local_endpoint(record.endpoint)) continue;
            ...
            if (now - last_address_refresh >= policy_.address_refresh_interval) {
                advertise_self();
                send_message(socket.get(), "getaddr");
                last_address_refresh = now;
            }
