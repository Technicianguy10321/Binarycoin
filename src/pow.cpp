...
std::uint32_t target_to_compact(const boost::multiprecision::cpp_int& target) {
    if (target <= 0 || target > uint256_maximum()) {
        throw std::invalid_argument("Target is outside the positive 256-bit range");
    }

    std::uint32_t size = static_cast<std::uint32_t>(
        (boost::multiprecision::msb(target) + 8U) / 8U);
    std::uint32_t compact = 0;

    if (size <= 3U) {
        const auto value = target.convert_to<std::uint64_t>();
        compact = static_cast<std::uint32_t>(value << (8U * (3U - size)));
    } else {
        compact = static_cast<std::uint32_t>(
            (target >> (8U * (size - 3U))).convert_to<std::uint64_t>());
    }
