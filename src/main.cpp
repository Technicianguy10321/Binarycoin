...
void print_help() {
    std::cout << R"HELP(BinaryCoin Testnet Alpha v0.1.5
...
Notes:
  ...
  - v0.1.5 advertises reachable local endpoints and dynamically dials peers learned through addrv2.
        ...
        if (command == "print-chain") {
            ...
            for (const bincoin::StoredBlock& stored : chain.blocks()) {
                ...
                std::cout << "height=" << stored.height
                          ...
                          << " bits=0x" << std::hex << stored.block.header.bits << std::dec
